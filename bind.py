...
class NightbotClientError(Exception):
...
class NightbotAPIError(Exception):
...
def bind_method(**config):
    ...
    class NightbotAPIMethod(object):
        ...
        def _build_parameters(self, args, kwargs):
            ...
            for index, value in enumerate(args):
                ...
                try:
                    ...
                    raise NightbotClientError("Too many arguments supplied")

            for key, value in six.iteritems(kwargs):
                if value is None:
                    continue
                if key in self.parameters:
                    raise NightbotClientError("Parameter %s already supplied" % key)
        ...
        def _do_api_request(self, url, method="GET", body=None, headers=None):
            ...
            if response['status'] == '503' or response['status'] == '429':
                raise NightbotAPIError(response['status'], "Rate limited", "Your client is making too many request per second")
            ...
            try:
                ...
                raise NightbotClientError('Unable to parse response, not valid JSON.', status_code=response['status'])
            ...
            if 'meta' not in content_obj:
                if content_obj.get('code') == 420 or content_obj.get('code') == 429:
                    ...
                    raise NightbotAPIError(content_obj.get('code'), "Rate limited", error_message)
                raise NightbotAPIError(content_obj.get('code'), content_obj.get('error_type'), content_obj.get('error_message'))
            api_responses = []
            ...
            if status_code == 200:
                ...
                # elif self.response_type == 'status':
                    #### TODO: if necessary, add status type response here
                ...
                raise NightbotAPIError(status_code, content_obj['meta']['error_type'], content_obj['meta']['error_message'])
    ...
    def _call(api, *args, **kwargs):
        method = NightbotAPIMethod(api, *args, **kwargs)
