...
class Channel(ApiModel):
    ...
    def __init__(self, id, name, displayName, joined, plan):
        self.id = id
        self.name = name
...
class Command(ApiModel):
    ...
    def __init__(self, id, createdAt, updatedAt, name, message, coolDown, count):
        self.id = id
...
class User(ApiModel):
    ...
    def __unicode__(self):
        return "%s User: %s" % (self.provider, self.displayName)

class Scope(User): #TODO: ... fix this somehow. Just... do it better ####
...
class Authorization(ApiModel):
    ...
    def object_from_dictionary(cls, entry):
        ...
        if 'credentials' in entry:
            credentials = Credential(timestamp_to_datetime(entry.get('expires')),
                                     entry.get('client'))
        # id = entry['id']
        return Authorization(id=id, userLevel=userLevel, authType=authType, credentials=credentials)

    def __unicode__(self):
        return "%s Authorization: [level:%s] %s" % (self.authType, self.userLevel, self.credentials)
